...
class ASTRunner:
  ...
  def launch_dims(self, var_vals, symbolic_cache=None):
    global_size = ([sym_infer(sz, var_vals, symbolic_cache) for sz in self.global_size] + [1]*(3-len(self.global_size))) if self.global_size is not None else self.global_size
    local_size = ([sym_infer(sz, var_vals, symbolic_cache) for sz in self.local_size] + [1]*(3-len(self.local_size))) if self.local_size is not None else self.local_size
