...
class Node:
  ...
  def expand(
    ...
    # print(self.legal_actions, policy_logits.shape)
...
class Roots:
  ...
  def __init__(self, root_num: int, legal_actions_list: List) -> None:
    ...
    for i in range(self.root_num):
      if isinstance(legal_actions_list, list) or isinstance(legal_actions_list, np.ndarray):
  ...
  def prepare(
    ...
    """
    value_prefixs = [0.0 for _ in range(self.root_num)]  # todo remove
    for i in range(self.root_num):
