...
from extra.optimization.transformerxl.config import Config
from extra.optimization.transformerxl.decoder import DecoderLayer


# from https://github.com/augustwester/transformer-xl/tree/main/transformer-xl
...
class TransformerXL(nn.Module):
    ...
    def default_config():
        ...
        config.embed_dim = 32
        config.seg_len = 39
        config.mem_len = 100 # not used for now
        config.num_heads = 4
        config.dropout = 0
    ...
    def forward(self, x, get_last_output=True):
        ...
        for i, dec in enumerate(self.layers):
            # x_ = x.clone()
            x = dec(x, None)# no memory use for now
            # x = dec(x, self.mem[i])
